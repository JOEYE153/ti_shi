	...
	<view class="dati-box">
		<view class="dati-box-switch-box">
			<view class="dati-box-switch">
				...
				<u-count-down v-if="!isSub" bg-color="$u-bg-color" :timestamp="timestamp" @end="countDownEnd" separator-size="50"
					font-size="50"></u-count-down>
					...
					<view class="swiper-item">
						...
						<view class="timu-choice-list">
							<dati-item choice="A" v-if="questions.option_a != ''" :text="questions.option_a" @click="changemode('A')"
								:mode="a_answer_mode_now" :choice_mode="a_answer_choice_mode_now" :is_multi="questions.is_multi"></dati-item>
							<dati-item choice="B" v-if="questions.option_b != ''" :text="questions.option_b" @click="changemode('B')"
								:mode="b_answer_mode_now" :choice_mode="b_answer_choice_mode_now" :is_multi="questions.is_multi"></dati-item>
							<dati-item choice="C" v-if="questions.option_c != ''" :text="questions.option_c" @click="changemode('C')"
								:mode="c_answer_mode_now" :choice_mode="c_answer_choice_mode_now" :is_multi="questions.is_multi"></dati-item>
							<dati-item choice="D" v-if="questions.option_d != ''" :text="questions.option_d" @click="changemode('D')"
								:mode="d_answer_mode_now" :choice_mode="d_answer_choice_mode_now" :is_multi="questions.is_multi"></dati-item>
						</view>
...
async favClick(){
			...
			async confirmSub() {
				...
				uni.setStorage({
					...
					success: function() {
						uni.redirectTo({
							url: '/pages/dati/finish?score='+ score + '&time=' + time+'&error=' + err + '&pass='+pass
						})
					}
				})
			},
			...
			submitAnswer() {
				this.mode[this.index] = 'submit'
				var answer_commit = '';
				if (this.a_answer_mode[this.index] == "checked") {
					if (answer_commit == '') {
				...
				if (this.b_answer_mode[this.index] == "checked") {
					if (answer_commit == '') {
				...
				if (this.c_answer_mode[this.index] == "checked") {
					if (answer_commit == '') {
				...
				if (this.d_answer_mode[this.index] == "checked") {
					if (answer_commit == '') {
				...
				this.$http({
					...
					success: (res) => {
						...
						if ((this.a_answer_mode[this.index] == "checked" && this.a_mode[this.index] ==
								"default")) {
							this.a_answer_mode[this.index] = "wrong"
						} else if (this.a_mode[this.index] == "right") {
							this.a_answer_mode[this.index] = "right"
						} else {
							this.a_answer_mode[this.index] = "default"
						}
						if ((this.b_answer_mode[this.index] == "checked" && this.b_mode[this.index] ==
								"default")) {
							this.b_answer_mode[this.index] = "wrong"
						} else if (this.b_mode[this.index] == "right") {
							this.b_answer_mode[this.index] = "right"
						} else {
							this.b_answer_mode[this.index] = "default"
						}
						if ((this.c_answer_mode[this.index] == "checked" && this.c_mode[this.index] ==
								"default")) {
							this.c_answer_mode[this.index] = "wrong"
						} else if (this.c_mode[this.index] == "right") {
							this.c_answer_mode[this.index] = "right"
						} else {
							this.c_answer_mode[this.index] = "default"
						}
						if ((this.d_answer_mode[this.index] == "checked" && this.d_mode[this.index] ==
								"default")) {
							this.d_answer_mode[this.index] = "wrong"
						} else if (this.d_mode[this.index] == "right") {
							this.d_answer_mode[this.index] = "right"
						} else {
							this.d_answer_mode[this.index] = "default"
						}
						this.updateMode()
