	...
	<view class="dati-box">
		<view class="dati-box-switch-box">
			<view class="dati-box-switch">
				<uni-segmented-control  :values="items"
				styleType="button" activeColor="#7468BE" activeTextColor="#fff" inactiveTextColor="#7468be" @clickItem="onClickItem"></uni-segmented-control>
			</view>
		</view>
				...
				<scroll-view scroll-y="true" >
					<view class="swiper-item">						
							...
							<view class="timu-choice-list">
								<template v-if="current == 1">  <!-- 是背题模式 -->
									<view class="timu-choice-list">
										<dati-item choice="A" v-if="questions.option_a != ''" :text="questions.option_a" 
										:mode="a_mode"  :choice_mode="a_mode" :is_multi="questions.is_multi"></dati-item>
										<dati-item choice="B" v-if="questions.option_b != ''" :text="questions.option_b" 
										:mode="b_mode" :choice_mode="b_mode" :is_multi="questions.is_multi"></dati-item>
										<dati-item choice="C" v-if="questions.option_c != ''" :text="questions.option_c" 
										:mode="c_mode" :choice_mode="c_mode" :is_multi="questions.is_multi"></dati-item>
										<dati-item choice="D" v-if="questions.option_d != ''" :text="questions.option_d" 
										:mode="d_mode" :choice_mode="d_mode" :is_multi="questions.is_multi"></dati-item>
									</view>
								</template>
								<template v-else>  <!-- 刷题模式 -->
									<view class="timu-choice-list">
										<dati-item choice="A" v-if="questions.option_a != ''" :text="questions.option_a" 
										@click="changemode('A')" :mode="a_answer_mode" :choice_mode="a_answer_choice_mode" :is_multi="questions.is_multi"></dati-item>
										<dati-item choice="B" v-if="questions.option_b != ''" :text="questions.option_b" 
										@click="changemode('B')" :mode="b_answer_mode" :choice_mode="b_answer_choice_mode" :is_multi="questions.is_multi"></dati-item>
										<dati-item choice="C" v-if="questions.option_c != ''" :text="questions.option_c" 
										@click="changemode('C')" :mode="c_answer_mode" :choice_mode="c_answer_choice_mode" :is_multi="questions.is_multi"></dati-item>
										<dati-item choice="D" v-if="questions.option_d != ''" :text="questions.option_d" 
										@click="changemode('D')" :mode="d_answer_mode" :choice_mode="d_answer_choice_mode" :is_multi="questions.is_multi"></dati-item>
									</view>
								</template>
							</view>				
		...
		<view class="dati-bottom">
			...
			<view class="dati-bottom-collection">
				<uni-fav :checked="checked" class="favBtn" circle="true" bgColor="#ffffff" fgColor="#7468BE" fgColorChecked="#ffffff" bgColorChecked="#7468BE" @click="favClick"/>
			</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			var _self = this;
			var gotoURL;
			if(!this.is_random){ // 不随机
				this.$http({
					url: '/question/display/',
					data:{
						id: nowid,
				...
				this.$http({
					url: '/question/display_random/' + getApp().globalData.courseId ,
					success: (res) => {
						if (res.data.code == 0){
							_self.$refs.popup.open('top')
						}else{
		...
		methods: {
			gotoPre(){
				...
				if(this.questions2.length == 0){
					...
					nextId = this.questions2[this.index].id
				}
				this.update(nextId)
